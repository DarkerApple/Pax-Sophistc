...
test('every event has a Korean title and body', () => {
  ...
  for (const event of EVENTS) {
    ...
    } else if (event.id === 'commodity-shock' || event.id === 'coup-attempt') {
      // These pick between two bodies, so both variants must exist.
      assert.ok(source.includes(`'event.${event.id}.`), `event ${event.id} variants are untranslated`);
    } else {
      assert.ok(source.includes(`'event.${event.id}'`), `event ${event.id} body is untranslated`);
    }
  }
});
